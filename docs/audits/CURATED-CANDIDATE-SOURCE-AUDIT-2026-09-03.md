...
- last-good 仅保存无凭据的公开字段、固定 commit SHA，以及成功获取的包与 GitHub 仓库元数据。远程 URL 会清除 userinfo、query 和 fragment；stdio 参数只保留 flag 名、参数数量及脱敏计数，不保存参数值。纵深检查在写 artifact、Issue 或快照前再次阻止原始 `args` 和凭据形态。
- 在线源临时失败时保留旧条目并明确标注 `last-good`，不会把旧结果伪装成实时结果，也不会因一次查询失败覆盖或删除既有快照记录。
