...
## 2026-09-20 DSH Web 本机运行验收（未完成正式上架条件）

用户允许在当前 `workspace` 安装测试卡片并启动授权；测试描述文件仅通过本机临时
`127.0.0.1` 服务提供，测试结束后已停止该服务。未将草案迁入 `connectors/`，
也未向公共 Registry 提交卡片。

| 候选 | 本机观察 | 后续状态 |
|---|---|---|
| 金数据 | DSH Web v0.2.51 显示项目范围 `workspace` 中“OAuth 已连接”，最近可用性检查 `ok`；`tools/list` 缓存显示 1 个服务、60 个工具。没有执行 `tools/call` 或读取表单、提交者数据。 | 仅连接与工具发现通过。创建、编辑、删除等工具在 UI 中仍显示“已允许 · 默认策略”，须先审查最小权限及逐工具策略，再用专用测试表单执行最小只读调用。测试卡片详情还显示“暂无预设 Prompt”，需核对动态导入与正式卡片的 Prompt 行为。 |
| Trello | 已进入 Atlassian OAuth 流程，但授权页提示 `No Trello workspaces found`；本机未确认成功安装或连接。 | 当前测试账号缺少 Trello 工作区，不能据此判定协议失败。用户要求跳过 Atlassian 测试，本轮不换号、不创建工作区、不重试。 |
