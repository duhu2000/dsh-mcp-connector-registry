...
|---|---|---|---|
| `shopline-developer-mcp` | SHOPLINE 官方 npm 包，MIT，stdio 免鉴权；`initialize` 和 9 个工具的 `tools/list` 已通过 | 工具描述包含行为指令，且有一个可对外发送反馈的工具；尚未完成人工批准 | 完成工具描述不可信边界复核，卡片明确反馈工具需用户同意，并由维护者批准描述符和版本固定策略 |
